...
<Tabs>
    <TabItem label='cURL'>
        ```shell curl http://osticket.homestead.local/api/agents -u "email:password" -d amount=1200
        ```
    </TabItem>
    <TabItem label='PHP'>```shell POST /api/agents ```</TabItem>
</Tabs>
...
import {updateResponse, _updateResponse} from "../../../../mock/api/agents" -->
