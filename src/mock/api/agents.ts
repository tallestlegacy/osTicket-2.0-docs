...
function fillArray(value: number | any[], callback: () => any): any[] {
    return Array.from(typeof value === 'number' ? { length: value } : value).map(callback)
}

export const _updateResponse = {
    objects: {
        current_page: 1,
        data: fillArray(2, () => [
            {
                id: faker.number.int({ max: 10 }),
                user_id: faker.person.middleName(),
                flags: 0,
...
export const updateResponse = JSON.stringify(
    {
        objects: {
            current_page: 1,
            data: [
